...
june_endings AS (
    SELECT
        ...
        SUM(CAST(w."End Balance" AS DOUBLE)) AS end_balance
    FROM with_FY w
...
final_calc AS (
    ...
    LEFT JOIN activity a
        ON j.FY = a.FY
        -- AND j.UEID = a.UEID
        AND j."CFIS/ORG Code" = a."CFIS/ORG Code"
        AND j.CBID = a.CBID
...
SELECT
    ...
    ROUND(SUM(beginning_balance) * 4) / 4 AS beginning_balance,
    ROUND(SUM(total_earned) * 4) / 4 AS total_earned,
    -- SUM(total_earned) AS total_earned,
    -- SUM(total_used) AS total_used,
    ROUND(SUM(total_used) * 4) / 4 AS total_used,
    -- SUM(total_misc) AS total_misc,
    ROUND(SUM(total_misc) * 4) / 4 AS total_misc,
...
ORDER BY 
    FY DESC,
    -- "Agency Code",
    CAST("CFIS/ORG Code" as INT) ASC,
    CBID,
    "Leave Benefit ID",
    "Leave Benefit Name"
)
SELECT * FROM final
-- WHERE FY = '2024-2025' AND "CFIS/ORG Code" = '0555' AND CBID = 'S09' AND "Leave Benefit ID" = 'HI'
""").df()

#print(df.to_string(index=False))

# ✅ Write to CSV
output_path = r"C:/Users/cb1152/Downloads/OneDrive_2025-11-03/leave_earned_used.csv"
