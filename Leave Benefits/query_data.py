...
df = con.execute("""
    ...
    -- SELECT count(distinct UEID) FROM final WHERE SUBSTR("Leave Period", 6, 2) = '06' AND FY = '2024-2025' AND agency_code = '051'

    -- SELECT DISTINCT "Leave Period" FROM base WHERE CBID = 'E' LIMIT 10
                 
    -- SELECT "Leave Period", "Department Name", CBID, "Position Number", "Full Position Number" FROM base WHERE length("Full Position Number") != 13 LIMIT 100
                                      
    --SELECT FY, "Agency Code", count(distinct UEID) as Count FROM with_FY
    --    GROUP BY FY, "Agency Code"
    --            ORDER BY FY DESC, "Agency Code"
    
    -- SELECT count(*) from base   

    SELECT b.FY, agency_code, "CFIS/ORG Code", COUNT(DISTINCT b.UEID) AS employee_count
    FROM final b
    LEFT JOIN non_clas_employees n
                 ON b.UEID = n.UEID AND b.FY = n.FY
    WHERE n.UEID IS NOT NULL -- AND b.FY = '2024-2025' LIMIT 20
    GROUP BY b.FY, agency_code, "CFIS/ORG Code"
    ORDER BY b.FY DESC, agency_code;
...
output_path = r"C:/Users/cb1152/Downloads/OneDrive_2025-11-03/count_non_clas_employees.csv"
...
### Find rows where "Begin Balance" is non-numeric
#con = duckdb.connect(database='C:/Users/cb1152/Downloads/OneDrive_2025-11-03/combined_leave_data.duckdb')
#
#query = """
#SELECT DISTINCT "source_file"
#FROM all_comp_abs
#WHERE TRY_CAST("End Balance" AS DOUBLE) IS NULL
#  AND "End Balance" IS NOT NULL
#ORDER BY "source_file"
#"""
#
#
#begin_balance = con.execute(query).df()
#print(begin_balance.to_string(index=False))
